...
<div class="content-wrapper">
    ...
    {!! \App\Library\Html::linkBack(route('admin.ams.room.index')) !!}
        <div class="d-block">
            <h4 class="content-title">{{ strtoupper(__('New Room')) }}</h4>
        </div>
    ...
    <div class="card shadow-sm col-md-6">
        <div class="card-body py-sm-4">
            <form method="post" action="{{ route('admin.ams.room.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-12">
                        <div class="p-sm-3">
                            ...
                            <div class="form-group row @error('office_room') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="office_room"
                                        value="{{ old('office_room') }}" placeholder="{{ __('Office Room Name') }}"
                                        required>
                                    @error('office_room')
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control" name="responsible_person" required>
                                        ...
                                        @foreach($responsible_persons as $key => $responsible_person)
                                            <option value="{{ $responsible_person->id }}" {{(old("responsible_person") == $responsible_person->id) ? "selected" : ""}}>
                            ...
                            <div class="form-group row @error('discription') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <textarea type="text" class="form-control" rows="4" name="discription" placeholder="{{ __('Discription') }}">{{ old('discription') }}</textarea>
                                    @error('discription')
                                        <p class="error-message">{{ $message }}</p>
                                    @enderror
                                </div>
