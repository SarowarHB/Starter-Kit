...
@section('title', __('Update Stock Transfer'))
...
<div class="content-wrapper">
    ...
    <div class="content-header d-flex justify-content-start">
        ...
        <div class="d-block">
            <h4 class="content-title">{{ strtoupper(__('Update Stock Transfer')) }}</h4>
        </div>

    </div>

    <div class="card shadow-sm">
        <div class="card-body py-sm-4 col-md-6">
            <form method="post" action="{{ route('admin.ams.stock_transfer.update', $stock_transfer->id) }}"
                    ...
                    <div class="col-md-12">
                        <div class="p-sm-3">
                            ...
                            <div class="form-group row @error('stock_id') error @enderror">
                                <label class="col-sm-3 col-form-label required">{{ __('Stock ID') }}</label>
                                <div class="col-sm-9">
                                <select class="form-control" name="stock_id" id="stock_id" required>
                                    ...
                                    @foreach($stocks as $stock)
                                        <option value="{{ $stock->id }}" {{(old("stock_id", $stock_transfer->stock_id) == $stock->id) ? "selected" : ""}}>
                            ...
                            <div class="form-group row @error('previous_location') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="previous_location" id="previous_location"
                                        value="{{ old('previous_location', $stock_transfer->previous_location) }}" placeholder="{{ __('Previous Location') }}" required readonly>
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control" name="current_location" id="current_location" required>
                                        ...
                                        @foreach($locations as $value)
                                            <option value="{{ $value }}" {{(old("current_location", $stock_transfer->current_location) == $value) ? "selected" : ""}}>
                            ...
                            <div class="form-group row @error('previous_quantity') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="number" class="form-control" name="previous_quantity" id="previous_quantity"
                                        value="{{ old('previous_quantity', $stock_transfer->previous_quantity) }}" placeholder="{{ __('Previous Quantity') }}" required readonly>
                            ...
                            <div class="form-group row @error('current_quantity') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="number" class="form-control" name="current_quantity" id="current_quantity" @if($stock_transfer->stock->product->category->categoryType->entry_type == 1) readonly @endif
                                        value="{{ old('current_quantity', $stock_transfer->current_quantity) }}" placeholder="{{ __('Current Quantity') }}" required>
                            ...
                            <div class="form-group row @error('note') error @enderror">
                                    ...
                                    <div class="col-sm-9">
                                        <textarea type="text" class="form-control" name="note"
                                            value="{{ old('note', $stock_transfer->note) }}" placeholder="{{ __('Note') }}" rows="8" required ></textarea>
                                        @error('note')
                                            <p class="error-message">{{ $message }}</p>
                                        @enderror
                                    </div>
