...
@section('title', __('Update Category Type'))
...
<div class="content-wrapper">
    ...
    {!! \App\Library\Html::linkBack(route('admin.ams.category_type.index')) !!}
        <div class="d-block">
            <h4 class="content-title">{{ strtoupper(__('Update Category Type')) }}</h4>
        </div>
    ...
    <div class="card shadow-sm col-md-6">
        <div class="card-body py-sm-4">
            <form method="post" action="{{ route('admin.ams.category_type.update', $category_type->id) }}" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-12">
                        <div class="p-sm-3">

                            <input type="hidden" name="id" value="{{ $category_type->id }}">

                            <div class="form-group row @error('name') error @enderror">
                                <label class="col-sm-3 col-form-label required">{{ __('Name') }}</label>
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="name"
                                        value="{{ old('name', $category_type->name) }}" placeholder="{{ __('Name') }}"
                                        required>
                                    @error('name')
                            ...
                            <div class="form-group row @error('unique_key') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="unique_key"
                                        value="{{ old('unique_key', $category_type->unique_key) }}" placeholder="{{ __('Unique Key') }}"
                                        required>
                                    @error('unique_key')
                                        <p class="error-message">{{ $message }}</p>
                                    @enderror
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control" name="entry_type" required>
                                        ...
                                        @foreach(\App\Library\Enum::getCategoryEntryType() as $index => $value)
                                            <option value="{{ $index }}" {{(old("entry_type", $category_type->entry_type) == $index) ? "selected" : ""}}>
