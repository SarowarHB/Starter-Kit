...
<div class="content-wrapper">
    ...
    <div class="card shadow-sm">
        <div class="card-body py-sm-4">
            <form method="post" action="{{ route('admin.ams.category.update', $category->id) }}" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-6">
                        <div class="p-sm-3">
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control" name="entry_type" required>
                                        ...
                                        @foreach(\App\Library\Enum::getCategoryEntryType() as $index => $value)
                                            <option value="{{ $index }}" {{(old("entry_type", $category->entry_type) == $index) ? "selected" : ""}}>
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control" name="category_type_id" required>
                                        ...
                                        @foreach($category_types as $category_type)
                                            <option value="{{ $category_type->id }}" {{(old("category_type_id", $category->category_type_id) == $category_type->id) ? "selected" : ""}}>
