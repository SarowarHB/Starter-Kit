<?php


namespace App\Library\Services\Admin;

use App\Models\Expense;
use App\Models\User;
use Yajra\DataTables\Facades\DataTables;
use Exception;

class ExpenseService extends BaseService
{
    private function actionHtml($row)
    {
        $actionHtml = '';

        if($row->id){
            $actionHtml = '<a class="dropdown-item text-primary" href="' . route('admin.expense.general.show', $row->id) . '" ><i class="fas fa-eye"></i> View</a>
            <a class="dropdown-item text-primary" href="' . route('admin.expense.general.update', $row->id) . '" ><i class="far fa-edit"></i> Edit</a>';
        }
        else{
            $actionHtml = '';
        }

        return '<div class="action dropdown">
                    <button class="btn btn2-secondary btn-sm dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                       <i class="fas fa-tools"></i> Action
                    </button>
                    <div class="dropdown-menu">
                        '.$actionHtml.'
                    </div>
                </div>';
    }

    public function dataTable()
    {
        $data = Expense::select('*');

            return DataTables::of($data)
                    ->addIndexColumn()
                    
                    ->addColumn('title', function($row){
                        return '<a class="text-success pr-2" href="' . route('admin.expense.general.show', $row->id) . '">'.$row->title.'</a>';
                    })
                    
                    ->addColumn('expense_type', function($row){
                        return ucwords($row->expense_type);
                    })
                    
                    ->addColumn('action', function($row){
                        return $this->actionHtml($row);
                    })
                    ->rawColumns(['title', 'expense_type', 'action'])
                    ->make(true);
    }

    public function createExpense(array $data): bool
    {
        try {
            $data['created_by'] = User::getAuthUser()->id;
            $this->data =  Expense::create($data);
            return $this->handleSuccess('Successfully Created');
        } catch (Exception $e) {
            return $this->handleException($e);
        }
    }

    public function updateExpense(Expense $expense, array $data): bool
    {
        try {
            $data['created_by'] = User::getAuthUser()->id;
            $this->data = $expense->update($data);

            return $this->handleSuccess('Successfully updated');
        }
        catch (Exception $e){
            return $this->handleException($e);
        }
    }

}
